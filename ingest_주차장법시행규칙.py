"""주차장법 시행규칙 PDF → law_articles ChromaDB 인덱싱 (국토교통부령 제1527호, 2026.03.31.)"""
...
PDF_MAIN  = BASE_DIR / "data/raw_laws/법령소스/주차장법 시행규칙(국토교통부령)(제01527호)(20260331).pdf"
PDF_BYEOL = BASE_DIR / "data/raw_laws/법령소스/주차장법 시행규칙 별표 (국토교통부령)(제01527호)(20260331).pdf"

LAW_NAME    = "주차장법 시행규칙"
LAW_ID      = "주차장법시행규칙"
LAW_TYPE    = "국토교통부령"
ENFORCEMENT = "20260331"
SOURCE_URL  = "https://www.law.go.kr/법령/주차장법시행규칙"
...
def parse_byeolpyo(text):
    text = clean_text(text)
    ms = list(re.finditer(r'■\s*주차장법\s*시행규칙\s*\[별표\s*(\d+(?:의\d+)?)\](?:\s*삭제[^\n]*)?\s*(?:<[^>]+>)?[ \t]*([^\n]*)?', text))
    arts = []
    for i, m in enumerate(ms):
        end = ms[i+1].start() if i+1 < len(ms) else len(text)
        arts.append({"law_id": LAW_ID, "law_name": LAW_NAME, "law_type": LAW_TYPE,
...
def main():
    ...
    print(f"\n총 {len(all_arts)}개")
    out = BASE_DIR / "data/raw_laws/주차장법시행규칙_articles.jsonl"
