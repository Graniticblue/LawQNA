...
_SCAN_NAME_BLOCK = ["특별회계", "기금", "위원회", "인권", "수당", "제설", "급수",
                    "감사", "포상", "표창", "수수료"]
# 핵심 기준형 조례 정확 명칭({지역}+아래) — 후보 절단 시 최우선 보존.
# 무순위 [:12] 절단이 '남양주시 주택 조례'를 버리고 수수료 조례를 남긴 실사고의 대책.
_SCAN_EXACT_CORE = ["주택 조례", "건축 조례", "건축 조례 시행규칙",
                    "도시계획 조례", "도시계획 조례 시행규칙",
                    "공동주택 관리 조례", "공동주택관리 조례",
                    "주차장 설치 및 관리 조례",
                    "도시 및 주거환경정비 조례", "도시 및 주거환경 정비 조례",
                    "경관 조례", "도시공원 조례", "도시공원 및 녹지 조례"]
# 차순위 판별용 핵심 토큰
_SCAN_CORE_TOKENS = ["주택", "건축", "도시계획", "공동주택", "주차", "정비", "경관", "공원"]
...
async def on_region_ordinance_scan(action: cl.Action):
    ...
    def _scan_register():
        ...
        # 전 키워드를 끝까지 수집한 뒤 주제 적합도로 랭킹 — 중간 절단하면
        # 뒤 키워드의 핵심 조례(주택 조례 등)가 노이즈에 밀려 버려진다.
        cands: dict = {}
        for kw in _REGION_SCAN_TOPICS:
            for l in law_api_fetcher.search_ordinances(f"{region} {kw}", display=100):
        ...
        def _rank(nm: str):
            ...
            for i, core in enumerate(_SCAN_EXACT_CORE):
                if n == rn + _norm(core):
                    return (0, i)          # 핵심 기준형 정확 명칭 — 절대 안 잘림
            for i, tok in enumerate(_SCAN_CORE_TOKENS):
                if tok in nm:
                    return (1, i)          # 핵심 토큰 포함
            return (2, 99)
    ...
    # 질문 주제와의 매칭(2-gram 겹침) — 등록 조례가 질문 내용을 직접 규정하는지
    # 감지해 표시한다. '조례를 등록하고도 관련 조문을 못 짚는' 문제의 가시화.
    def _bi(s):
        s = re.sub(r"[^가-힣]", "", s)
        return {s[i:i + 2] for i in range(len(s) - 1)}

    qb = _bi(query) if query else set()
    matches: dict = {}          # nm → [(score, art_no, text)]
    if qb:
    ...
    actions = []
    if query:
        # 질문 관련 조문을 재생성 자료로 적재 (위 매칭 결과 재사용)
        blocks = [f"[출처: {region} 조례 자동 스캔·캐싱]"]
        budget = 12000
        for nm, _, _, _arts in registered:
            for _sc, a, t in matches.get(nm, []):
                if budget <= 0:
                    break
                piece = f"[조례원문] 「{nm}」 {a}\n{t[:3000]}"
                blocks.append(piece)
