"""주차장법 시행령 PDF → law_articles ChromaDB 인덱싱 (대통령령 제35708호, 2025.08.17.)"""
...
PDF_MAIN  = BASE_DIR / "data/raw_laws/법령소스/주차장법 시행령(대통령령)(제35708호)(20250817).pdf"
PDF_BYEOL = BASE_DIR / "data/raw_laws/법령소스/주차장법 시행령 별표 (대통령령)(제35708호)(20250817).pdf"

LAW_NAME    = "주차장법 시행령"
LAW_ID      = "주차장법시행령"
LAW_TYPE    = "대통령령"
ENFORCEMENT = "20250817"
SOURCE_URL  = "https://www.law.go.kr/법령/주차장법시행령"
...
def parse_byeolpyo(text):
    text = clean_text(text)
    ms = list(re.finditer(r'■\s*주차장법\s*시행령\s*\[별표\s*(\d+(?:의\d+)?)\](?:\s*삭제[^\n]*)?\s*(?:<[^>]+>)?[ \t]*([^\n]*)?', text))
    arts = []
    for i, m in enumerate(ms):
        end = ms[i+1].start() if i+1 < len(ms) else len(text)
        arts.append({"law_id": LAW_ID, "law_name": LAW_NAME, "law_type": LAW_TYPE,
...
def ingest_to_chroma(articles):
    ...
    for rec in articles:
        ...
        meta = {k: rec[k] for k in ("law_id","law_name","law_type","enforcement_date","source_url")}
...
def main():
    ...
    print(f"\n총 {len(all_arts)}개")
    out = BASE_DIR / "data/raw_laws/주차장법시행령_articles.jsonl"
