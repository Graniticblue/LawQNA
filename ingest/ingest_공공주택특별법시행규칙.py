"""공공주택 특별법 시행규칙 PDF → law_articles ChromaDB 인덱싱 (국토교통부령 제1552호, 2025.12.29.)"""
...
PDF_MAIN  = BASE_DIR / "data/raw_laws/법령소스/공공주택 특별법 시행규칙(국토교통부령)(제01552호)(20251229).pdf"
PDF_BYEOL = BASE_DIR / "data/raw_laws/법령소스/공공주택 특별법 시행규칙 별표 (국토교통부령)(제01552호)(20251229).pdf"

LAW_NAME    = "공공주택 특별법 시행규칙"
LAW_ID      = "공공주택특별법시행규칙"
LAW_TYPE    = "국토교통부령"
ENFORCEMENT = "20251229"
SOURCE_URL  = "https://www.law.go.kr/법령/공공주택특별법시행규칙"
...
def parse_byeolpyo(text):
    text = clean_text(text)
    ms = list(re.finditer(
        r'■\s*공공주택\s*특별법\s*시행규칙\s*\[별표\s*(\d+(?:의\d+)?)\](?:\s*삭제[^\n]*)?\s*(?:<[^>]+>)?[ \t]*([^\n]*)?',
        text
    ))
    arts = []
    for i, m in enumerate(ms):
...
def main():
    ...
    print(f"\n총 {len(all_arts)}개")
    out = BASE_DIR / "data/raw_laws/공공주택특별법시행규칙_articles.jsonl"
