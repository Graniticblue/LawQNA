"""건설기술 진흥법 시행규칙 PDF → law_articles ChromaDB 인덱싱 (국토교통부령 제1531호, 2025.10.31.)"""
...
PDF_MAIN  = BASE_DIR / "data/raw_laws/법령소스/건설기술 진흥법 시행규칙(국토교통부령)(제01531호)(20251031).pdf"
PDF_BYEOL = BASE_DIR / "data/raw_laws/법령소스/건설기술 진흥법 시행규칙 별표 (국토교통부령)(제01531호)(20251031).pdf"

LAW_NAME    = "건설기술 진흥법 시행규칙"
LAW_ID      = "건설기술진흥법시행규칙"
LAW_TYPE    = "국토교통부령"
ENFORCEMENT = "20251031"
SOURCE_URL  = "https://www.law.go.kr/법령/건설기술진흥법시행규칙"
...
def parse_byeolpyo(text):
    text = clean_text(text)
    ms = list(re.finditer(
        r'■\s*건설기술\s*진흥법\s*시행규칙\s*\[별표\s*(\d+(?:의\d+)?)\](?:\s*삭제[^\n]*)?\s*(?:<[^>]+>)?[ \t]*([^\n]*)?',
        text
    ))
    arts = []
    for i, m in enumerate(ms):
...
def main():
    ...
    print(f"\n총 {len(all_arts)}개")
    out = BASE_DIR / "data/raw_laws/건설기술진흥법시행규칙_articles.jsonl"
