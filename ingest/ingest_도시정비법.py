"""도시 및 주거환경정비법 본법 PDF → law_articles ChromaDB 인덱싱"""
import re, json, time
from pathlib import Path
import fitz

BASE_DIR = Path(__file__).parent
PDF_MAIN = BASE_DIR / "data/raw_laws/법령소스/도시 및 주거환경정비법(법률)(제21065호)(20260102).pdf"

LAW_NAME    = "도시 및 주거환경정비법"
LAW_ID      = "도시정비법"
LAW_TYPE    = "법률"
ENFORCEMENT = "20260102"
SOURCE_URL  = "https://www.law.go.kr/법령/도시및주거환경정비법"
...
def main():
    print(f"[1] 본법 파싱: {PDF_MAIN.name}")
    text = extract_text(PDF_MAIN)
    arts = parse_articles(text)
    print(f"    조문 {len(arts)}개 파싱됨")
    for a in arts[:3]:
        print(f"    {a['article_no']}({a['article_title']}): {a['content'][:60]}...")

    out = BASE_DIR / "data/raw_laws/도시정비법_articles.jsonl"
    out.parent.mkdir(parents=True, exist_ok=True)
    with open(out, "w", encoding="utf-8") as f:
        for a in arts:
            f.write(json.dumps(a, ensure_ascii=False) + "\n")
    print(f"JSONL 저장: {out} ({len(arts)}건)")
    ingest_to_chroma(arts)
