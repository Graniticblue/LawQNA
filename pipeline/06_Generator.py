...
def _load_statute_quote():
    ...
    답변 속 조문 직접 인용을 로컬 코퍼스 원문과 대조하는 모듈(16-0506 시제
    반전 검증에서 판정자 오판을 잡아낸 층). 로드 실패 시 None — 검증은
    비차단이므로 답변 흐름을 깨지 않는다."""
    global _statute_quote_mod
    if _statute_quote_mod is None:
        try:
            import importlib.util
            p = Path(__file__).parent.parent / "ingest" / "statute_quote.py"
            spec = importlib.util.spec_from_file_location("statute_quote", p)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            _statute_quote_mod = mod
        except Exception:
            _statute_quote_mod = False
    return _statute_quote_mod or None
...
2. **[캐싱·첨부 자료 — 답변 재생성용]** 또는 **[API 패치 자료]** 섹션 (extra_context로 주입된 경우): 사각지대 법령을 법제처 API로 실시간 패치했거나 사용자가 직접 첨부한 raw 조문입니다. 이 자료를 [직접 참조 조문]과 동급으로 신뢰하여 1차 근거로 활용하세요.
3. **[관련 법령 조문]** 섹션: 유사도 검색으로 찾은 조문. 적용 가능하면 사용하세요.
4. **[유사 질의회신 선례]** 섹션: 관련 있으면 반드시 인용하세요.
   - **실무 확인 사례 제시 의무**: 사안과 사실관계가 유사한 부처·지자체 회신(국토교통부·서울시 등)이
     검색되면, 법적 근거(법제처 해석례·판례)와 **별도로 반드시 제시**한다. 이런 회신의 가치는 권위가
     아니라 **"실제로 이렇게 처리·결정된 사례가 있다"는 확인 기능**이다 — 사용자가 제시한 케이스와
     유사한 실결정례의 존재 자체가 유용하므로, 권위가 낮다는 이유로 생략하지 않는다.
   - 프레이밍: `"실무상 유사 사례로, 국토교통부 2017.11.21. 회신(녹색건축과-6804)은 ~라고 회신한
     바 있습니다."` 처럼 실무 사례임을 밝히고 제시한다.
   - **위계 규칙**: 부처·지자체 회신의 결론이 법제처 해석례·대법원 판례와 충돌하면 상위(법제처·판례)를
     결론의 근거로 삼되, 실무 회신의 존재와 차이를 언급한다. 지자체 회신이 **해당 지자체 조례의 해석**인
     경우에는 그 조례에 관한 한 준(準)주근거로 취급한다.
...
올바른 인용 형식 (△·○는 자리표시자 — [참조 자료 목록]에 **실제 존재하는 번호로만** 채운다. 목록에 없으면 선례 인용 자체를 하지 않는다):
- **해석례** — `"법제처 △△-△△△△에 따르면 ~"` 또는 `"법제처 △△-△△△△ 해석례는 ~"` 또는 `"(법제처 △△-△△△△)"`
- **부처·지자체 질의회신** (법제처 번호가 없는 선례) — [유사 질의회신 선례]의 해당 항목 헤더에 적힌 **(인용 표기: "...")를 글자 그대로** 사용: 예 `"국토교통부 2012.07.30. 회신/서울시질의회신집에 따르면 ~"` 또는 `"(국토교통부 민원회신-서울특별시 2017.11.21.)"`. 임의 축약·변형(`(관련 국토교통부 회신)`, 출처집 표기 생략 등) 금지 — 표기가 다르면 출처 링크가 걸리지 않는다.
...
- ✅ `"대법원 ○○○○두○○○○ 판결도 같은 맥락에서 예외 규정의 엄격해석을 확인한 바 있습니다."` (목록에 있는 사건번호로만 — 목록에 없으면 판례 존재 주장 없이 해석 논리로만 서술)
...
2. **[참조 자료 목록]에 없는 선례는 '존재 주장' 자체를 금지.**
   목록에 없는 회신·해석례·판례가 존재한다고 시사하는 표현 일체를 쓰지 마라 —
   식별번호가 없으면 사용자가 확인할 수 없어 거짓 인용과 동급이다.
   - ❌ "법제처는 ~라고 해석한 바 있습니다(관련 국토교통부 회신)"
   - ❌ "~는 것이 법제처의 일관된 입장입니다" (번호 없는 익명 권위) — 그 원칙의
     근거 해석례가 [참조 자료 목록]에 있으면 반드시 그 번호로 실명 인용하고,
     없으면 '입장·판례·해석례의 존재'를 주장하지 말고 해석 논리로만 서술하라
   - ❌ "(관련 회신)", "(관련 해석례)", "~로 회신된 바 있습니다", "일관되게 이러한 입장을 취해왔습니다"
   - ✅ 같은 결론을 **선례 원용 없이 해석 논리로** 서술: "피난 안전 확보라는 입법 취지에 비추어 ~라고 해석함이 타당합니다"
   - 또는 그 단락 자체를 생략

3. **자가 검증 원칙.** 답변에 사건번호·해석례 번호·회신 표기를 적기 전, 그것이 [참조 자료 목록]·[유사 질의회신 선례]에 실제로 있는지 확인하라. 없으면 그 선례 언급을 삭제하고 해석 논리로 대체한다.

4. **조문 문언 날조 금지.** 조문을 따옴표로 직접 인용할 때는 [관련 법령 조문] 등 컨텍스트에 있는 **원문 문자열 그대로만** 쓴다. 원문에 없는 괄호·단서·한정 문구를 끼워 넣어 요건을 창설하는 것은 거짓 사건번호와 동급의 날조다(시스템이 답변의 모든 조문 인용을 원문과 자동 대조한다). 원문에 없는 내용을 말하려면 인용부호 밖에서 해석 의견임을 밝히고 서술하라. 조문 내용을 기억으로 재구성하지 말 것 — 컨텍스트에 원문이 없으면 인용부호를 쓰지 않는다.
...
- **DB-조문**: [참조 자료 목록] 또는 [관련 법령 조문]·[법령원문]·[API 패치 자료] 섹션에 **실제 있는** 조문만. 본문에 인용한 조문이 그 목록에 있는지 확인 후 표시.
- **DB-선례**: [유사 질의회신 선례] 섹션에 실제 있는 해석례만.
- **DB-입법요지**: [관련 개정연혁] 섹션의 개정이유·키포인트를 실제로 활용한 경우만.
- **⚠ 법률서치필요**: 시스템에서 **[사각지대 법령]으로 지정된 법령**을 본문에 추정 인용한 경우 (DB·API 패치 자료에 없음). 시스템 프롬프트의 `## 사각지대 법령 안내` 섹션에 명시된 법령을 본문에 인용했으면 **반드시 여기에 표시**.
...
# 「법령명」[ 제N조] 에서 법령류만 — 고시·기준·지침·계획 등은 API 패치 대상이 아니라 제외.
...
# ============================================================
# 지자체 실명 사전 — 질문 텍스트에서 지역 직접 탐지 (사실표 의존 제거)
#   pass1 사실표에 지역이 빠지면 미보유 지역 감지가 통째로 죽는 문제의 대책.
#   사전 기반이라 '필요시/공사시/신도시' 같은 어미 오탐이 없다.
# ============================================================
...
_KNOWN_REGIONS = (
    ...
    "창녕군", "의령군", "함안군", "남해군", "하동군", "산청군", "함양군",
    "거창군", "합천군",
    "기장군", "달성군", "강화군", "옹진군", "울주군",
)

# 축약·통칭 → 정식 명칭 (자치법규명은 정식 명칭으로 시작하므로 스캔 매칭에 필수).
# '광주시'는 경기도 광주시가 실존해 광주광역시로 강제 변환하지 않는다.
_REGION_ALIAS = {
    "부산시": "부산광역시", "대구시": "대구광역시", "인천시": "인천광역시",
    "대전시": "대전광역시", "울산시": "울산광역시", "세종시": "세종특별자치시",
    "서울시": "서울특별시", "제주도": "제주특별자치도",
}

_KNOWN_REGION_PAT = re.compile(
    r"(?<![가-힣])("
    + "|".join(sorted(map(re.escape, _KNOWN_REGIONS + tuple(_REGION_ALIAS)),
                      key=len, reverse=True))
    + r")")


def _extract_query_regions(text: str) -> list[str]:
    """텍스트에 언급된 지자체(정식 명칭으로 정규화) — 긴 이름 우선 매칭이라
    '남양주시' 안의 '양주시' 같은 부분 문자열 오탐이 없다."""
    out: list[str] = []
    for m in _KNOWN_REGION_PAT.finditer(text or ""):
        nm = _REGION_ALIAS.get(m.group(1), m.group(1))
        if nm not in out:
            out.append(nm)
    return out
...
def strip_unverified_citations(
    ...
    allowed_labels: 검색된 선례의 공식 인용 표기(cite_label — 예 "국토교통부
    2012.07.30. 회신"). 날짜형·부서형 정규식이 지우기 전에 마스킹으로 보호한다 —
    이전에는 검색된 실제 선례의 정당한 날짜형 인용까지 무조건 "관련 ○○ 회신"으로
    뭉개져, DB에 있는 선례가 출처 없는 문장처럼 보이는 문제가 있었다.

    반환: (정제된 답변, 제거된 번호 목록).
    """
    removed: list[str] = []

    # ── 0) 정당한 인용 표기 보호 (마스킹 → 치환 후 복원) ──
    masks: dict[str, str] = {}
    for i, lab in enumerate(sorted(allowed_labels or (), key=len, reverse=True)):
        if lab and lab in answer:
            tok = f"\x00CITE{i}\x00"
            masks[tok] = lab
    ...
    def _agency(m):
        ...
        # doc_code가 순번만("7544")인 레거시 자료도 있어 두 형태 모두 대조
    ...
    # 장식형 익명 인용 제거: 괄호 하나에 익명화된 인용만 들어 있으면 괄호째 삭제.
    # 예: "…취지가 있습니다(관련 대법원 판례(존재여부 리서치 필요))." → 괄호 제거.
    # 익명 인용은 번호가 없어 '리서치'가 실행 불가능한 지시라 정보가치가 없고,
    # 취지·법리 일반론은 인용 없이 서술로 서는 편이 낫다. 문장 본문에 박힌
    # 인용("관련 대법원 판례(…)에 따르면")은 주장 전체가 미확인임을 알리므로 유지.
    out = re.sub(
        r"\s*\(\s*관련\s*[가-힣 ]{0,10}(?:판례|해석례|회신|행정해석)\s*"
        r"\(존재여부 리서치 필요\)\s*\)",
        "", out)

    for tok, lab in masks.items():
        out = out.replace(tok, lab)
    return out, removed
...
class Generator:
    ...
    def generate(self, query: str, verbose: bool = True, extra_context: str = "", session_id: str = "", stream_callback=None, provider: str = "gemini", as_of_date: str = "", exclude_doc_codes: set = None, as_of_code: str = "", thread_id: str = "", carry_laws: list = None, carry_conclusions: list = None) -> dict:
        ...
        # 지역 감지에 공용. ① 질문·맥락 텍스트의 지자체 실명(사전 탐지, 정규화)
        # ② 보유 팩 매칭 ③ pass1 사실표의 지역 값 — 셋의 합집합.
        # 사실표에만 의존하면 첫 질문에서 pass1이 지역을 빠뜨릴 때 감지가 죽는다
        # (부산 1230세대 실사례).
        query_regions: list = list(matched_regions)

        def _add_region(v: str):
            v = _REGION_ALIAS.get(v, v)
            if v and v not in query_regions:
                query_regions.append(v)

        for _nm in _extract_query_regions(f"{query}\n{extra_context}"):
            _add_region(_nm)
        ...
        if answer_mode == "산출":
            ...
            mode_note = (
                ...
                "- **질문이 '각 항목'을 물으면 해당하는 모든 항목을 빠짐없이 다룬다** — 일부만 골라 답하지 말 것. "
                "각 항목의 기준이 별개 개별 법령(예: 어린이집→「영유아보육법」, 작은도서관→「도서관법」)에 있으면, "
                "본문에서 그 항목에 어느 법령이 적용되는지 명시하고, 그 법령 원문이 제공되지 않았으면 "
                "'해당 법령의 구체 기준은 별도 확인 필요'라고 표시한다(막연히 넘어가지 말 것).\n"
...
🚫 조문 문언 날조 금지 — 조문을 따옴표로 인용할 때는 위 [검색된 관련 조문]의 원문 문자열 그대로만 쓴다(시스템이 원문과 자동 대조한다). 원문에 없는 괄호·단서를 삽입해 요건을 만들지 말고, 해석 의견은 인용부호 밖에서 서술하라."""
        ...
        def _sanitize(ans: str):
            ans, removed = strip_unverified_citations(ans, allowed_codes, allowed_labels)
            # 인용 괄호 끝의 '참조' 제거: "(법제처 12-0596 참조)" → "(법제처 12-0596)"
            ans = re.sub(r"\s*참조(?=\s*\))", "", ans)
            # 스트립 잔재("관련 국토교통부 회신" 등)가 출처 목록에 불릿으로만 남으면 제거
            # (식별번호 없는 환각 잔재라 출처로서 무의미). '(내장지식, ~원칙)' 같은
            # 꼬리 주석이 붙은 불릿도 동일 — 식별자가 없으면 출처가 아니다.
            # 문장 중간 인용은 보존.
            ans = re.sub(
                r"(?m)^[ \t>*\-•·]*관련\s*"
        ...
        answer, removed_cites = _sanitize(answer)
        if verbose and removed_cites:
            print(f"\n  [거짓인용 차단] {len(removed_cites)}건: {removed_cites}")

        # ── 조문 인용 검증 (로컬 원문 대조) — 문언 불일치 교정 ──
        # 답변이 따옴표로 '법문'이라 주장한 문자열을 코퍼스 원문과 대조한다.
        # 불일치 시 비스트리밍 경로(eval·CLI)는 실제 원문을 주입해 1회 교정
        # 재생성하고, 스트리밍 경로(chainlit)는 이미 본문이 나간 뒤이므로
        # findings만 반환해 UI 층(배지 + 재생성 버튼)이 처리한다.
        statute_findings: list = []
        statute_regen = False
        _sq = _load_statute_quote()
        if _sq is not None:
            try:
                statute_findings = _sq.verify_answer_quotes(answer)
            except Exception:
                statute_findings = []
            _mis = [f for f in statute_findings if f.get("status") == "mismatch"]
            if _mis and stream_callback is None:
                block = _sq.corrective_block(_mis)
                if block:
                    if verbose:
                        print(f"\n  [조문 인용 검증] 원문 불일치 {len(_mis)}건 — 실제 원문 주입 교정 재생성")
                    try:
                        answer = _call(pass2_system, pass2_input + "\n\n" + block)
                        answer, _removed2 = _sanitize(answer)
                        removed_cites = removed_cites + _removed2
                        statute_regen = True
                        statute_findings = _sq.verify_answer_quotes(answer)
                    except Exception:
                        pass  # 교정 실패 시 1차 답변 유지 — 검증은 비차단
        ...
        return {
            ...
            "statute_findings":  statute_findings,
            "statute_regen":     statute_regen,
            "law_docs":          law_docs,
            "qa_docs":           qa_docs,
            "case_docs":         case_docs,
