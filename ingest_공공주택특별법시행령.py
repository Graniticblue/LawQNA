"""공공주택 특별법 시행령 PDF → law_articles ChromaDB 인덱싱 (대통령령 제36258호, 2026.04.14.)"""
...
PDF_MAIN  = BASE_DIR / "data/raw_laws/법령소스/공공주택 특별법 시행령(대통령령)(제36258호)(20260414).pdf"
PDF_BYEOL = BASE_DIR / "data/raw_laws/법령소스/공공주택 특별법 시행령 별표 (대통령령)(제36258호)(20260414).pdf"

LAW_NAME    = "공공주택 특별법 시행령"
LAW_ID      = "공공주택특별법시행령"
LAW_TYPE    = "대통령령"
ENFORCEMENT = "20260414"
SOURCE_URL  = "https://www.law.go.kr/법령/공공주택특별법시행령"
...
def parse_byeolpyo(text):
    text = clean_text(text)
    ms = list(re.finditer(
        r'■\s*공공주택\s*특별법\s*시행령\s*\[별표\s*(\d+(?:의\d+)?)\](?:\s*삭제[^\n]*)?\s*(?:<[^>]+>)?[ \t]*([^\n]*)?',
        text
    ))
    arts = []
    for i, m in enumerate(ms):
...
def main():
    ...
    print(f"\n총 {len(all_arts)}개")
    out = BASE_DIR / "data/raw_laws/공공주택특별법시행령_articles.jsonl"
